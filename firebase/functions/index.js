  ...
  await firestore
    ...
    .then(async (querySnapshot) => {
      for (var doc of querySnapshot.docs) {
        console.log(`Deleting document ${doc.id} from collection comment`);
        await doc.ref.delete();
      }
    });
  await firestore
    .collection("interview_question")
    .where("author", "==", userRef)
    .get()
    .then(async (querySnapshot) => {
      for (var doc of querySnapshot.docs) {
        console.log(
          `Deleting document ${doc.id} from collection interview_question`,
        );
        await doc.ref.delete();
      }
    });
  await firestore
    .collection("comment_interview_question")
    .where("user", "==", userRef)
    .get()
    .then(async (querySnapshot) => {
      for (var doc of querySnapshot.docs) {
        console.log(
          `Deleting document ${doc.id} from collection comment_interview_question`,
        );
        await doc.ref.delete();
      }
    });
